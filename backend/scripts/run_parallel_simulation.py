...
- Does not close environment after simulation, enters command waiting mode
...
if sys.platform == 'win32':
    ...
    # This affects all open() calls without explicit encoding
...
class ParallelIPCHandler:
    ...
    async def _interview_single_platform(self, agent_id: int, prompt: str, platform: str) -> Dict[str, Any]:
        ...
        Returns:
            Dictionary with result, or dictionary with error
        """
...
# Non-essential action types to filter out (low analysis value)
...
def get_agent_names_from_config(config: Dict[str, Any]) -> Dict[int, str]:
    ...
    This allows showing real entity names in actions.jsonl instead of "Agent_0" style codes
...
def fetch_new_actions_from_db(
    ...
    Args:
        ...
        last_rowid: Last processed rowid value (use rowid instead of created_at because different platforms have different created_at formats)
...
def _enrich_action_context(
    ...
    try:
        ...
        # Create comment: add parent post information
...
def create_model(config: Dict[str, Any], use_boost: bool = False):
    ...
    # Select which LLM to use based on parameter and config availability
...
async def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help='Maximum simulation rounds (optional, to truncate long simulations)'
    )
    parser.add_argument(
        '--no-wait',
        action='store_true',
    ...
    if args.twitter_only:
        ...
        # Run in parallel (each platform uses independent action logger)
...
def setup_signal_handlers(loop=None):
    ...
    When receiving termination signal, needs to:
