...
class ReportLogger:
    ...
    def log(
        ...
        Args:
            ...
            details: Detailed content dict, no truncation
    ...
    def log_section_content(
        ...
        """Log section content generation complete (content only, not the entire section)"""
        self.log(
...
class ReportAgent:
    ...
    def _parse_tool_calls(self, response: str) -> List[Dict[str, Any]]:
        ...
        # Only try when format 1 does not match, avoid false matches in body text
    ...
    def _generate_section_react(
        ...
        # Build user prompt - each completed section passes max 4000 chars
        ...
        for iteration in range(max_iterations):
            ...
            if has_tool_calls:
                # Tool quota exhausted, explicitly request Final Answer output
                ...
                call = tool_calls[0]
                if len(tool_calls) > 1:
                    logger.info(f"LLM attempted {len(tool_calls)} tools, only executing first: {call['name']}")
...
class ReportManager:
    ...
    def get_console_log(cls, report_id: str, from_line: int = 0) -> Dict[str, Any]:
        ...
        Args:
            report_id: Report ID
            from_line: Starting line for reading (for incremental fetch, 0 means from start)
    ...
    def get_console_log_stream(cls, report_id: str) -> List[str]:
        """
        Get full console logs (one-time full fetch)
    ...
    def get_agent_log_stream(cls, report_id: str) -> List[Dict[str, Any]]:
        """
        Get full Agent logs (one-time full fetch)
    ...
    def _clean_section_content(cls, content: str, section_title: str) -> str:
        ...
        for i, line in enumerate(lines):
            ...
            # If previous line was a skipped heading and current line is blank, skip too
    ...
    def _post_process_report(cls, content: str, outline: ReportOutline) -> str:
        ...
        while i < len(lines):
            ...
            if heading_match:
                ...
                # Check for duplicate headings (same content heading within consecutive 5 lines)
                is_duplicate = False
                ...
                # Skip separator lines right after headings
                i += 1
                continue
