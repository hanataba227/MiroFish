...
# Adding this prefix prevents Agent from calling tools and forces text-only reply
...
def _check_simulation_prepared(simulation_id: str) -> tuple:
    ...
    try:
        ...
        # - completed: run completed, preparation was done earlier
...
def prepare_simulation():
    ...
    try:
            ...
            if is_prepared:
                ...
                return jsonify({
                    ...
                    "data": {
                        ...
                        "status": "ready",
                        "message": "Completed preparation work exists, no need to regenerate",
        ...
        # ========== Synchronous entity count retrieval (before background task starts) ==========
        # This way the frontend can immediately get the expected total Agent count after calling prepare
        try:
            logger.info(f"Synchronous entity count retrieval: graph_id={state.graph_id}")
            reader = ZepEntityReader()
...
def get_prepare_status():
    ...
    try:
        ...
        if not task:
            # Task does not exist, but if simulation_id available, check if prepared
            if simulation_id:
                is_prepared, prepare_info = _check_simulation_prepared(simulation_id)
                if is_prepared:
                    return jsonify({
...
def _get_report_id_for_simulation(simulation_id: str) -> str:
    ...
    Traverses the reports directory, finds reports matching simulation_id,
...
def get_simulation_history():
    ...
    Used for homepage historical project display, returns simulation list with rich info like project name and description
...
def download_simulation_script(script_name: str):
    """
    Download simulation run script files (common scripts, located in backend/scripts/)
...
def generate_profiles():
    ...
    try:
        ...
        if filtered.filtered_count == 0:
            return jsonify({
                "success": False,
                "error": "No entities matching the criteria found"
            }), 400
...
def start_simulation():
    ...
    About enable_graph_memory_update:
        ...
        - This lets the graph "remember" the simulation process for later analysis or AI dialogue
                ...
                if state.status == SimulationStatus.RUNNING:
                    ...
                    if run_state and run_state.runner_status.value == "running":
                        ...
                        if force:
                            ...
                            return jsonify({
                                "success": False,
                                "error": "Simulation is currently running. Please call /stop to stop it first, or use force=true to force restart"
                            }), 400
...
def interview_agent():
    ...
    Note: This function requires the simulation environment to be running (entered command-waiting mode after completing simulation loop)
...
def close_simulation_env():
    ...
    Note: This differs from /stop endpoint, /stop will forcefully terminate the process,
    while this endpoint gracefully closes the environment and exits.
