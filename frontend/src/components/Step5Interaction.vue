          ...
          <div class="action-bar-tabs">
            ...
            <div class="agent-dropdown" v-if="profiles.length > 0">
              ...
              >
                ...
                <span>{{ selectedAgent ? selectedAgent.username : '월드 내 개체와 대화' }}</span>
        ...
        <div v-if="activeTab === 'chat'" class="chat-container">
          ...
          <div v-if="chatTarget === 'report_agent'" class="report-agent-tools-card">
            <div class="tools-card-header">
              ...
              <div class="tools-card-info">
                ...
                <div class="tools-card-subtitle">보고서 생성 에이전트의 빠른 대화 버전, 4가지 전문 도구 호출 가능, MiroFish의 완전한 기억 보유</div>
              </div>
              ...
              <div class="tools-grid">
                ...
                <div class="tool-item tool-blue">
                  ...
                  <div class="tool-content">
                    ...
                    <div class="tool-desc">그래프 구조 기반 너비 우선 탐색 알고리즘으로 이벤트 전파 경로를 재구성하고, 전체 정보 흐름의 토폴로지 구조 포착</div>
                  </div>
                </div>
                <div class="tool-item tool-orange">
                  ...
                  <div class="tool-content">
                    ...
                    <div class="tool-desc">GraphRAG 기반 즉시 쿼리 인터페이스, 인덱스 효율 최적화, 구체적인 노드 속성 및 개별 사실의 빠른 추출에 사용</div>
                  </div>
                </div>
                <div class="tool-item tool-green">
                  ...
                  <div class="tool-content">
                    ...
                    <div class="tool-desc">자율형 인터뷰, 시뮬레이션 월드 내 개체와 병렬로 다중 라운드 대화 수행, 비구조화된 관점 데이터 및 심리 상태 수집</div>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div class="chat-messages" ref="chatMessages">
            <div v-if="chatHistory.length === 0" class="chat-empty">
              ...
              <p class="empty-text">
                {{ chatTarget === 'report_agent' ? 'Report Agent와 대화하여 보고서 내용을 심층적으로 이해하세요' : '시뮬레이션 개체와 대화하여 그들의 관점을 알아보세요' }}
              </p>
            </div>
            <div 
          ...
          <div class="survey-setup">
            <div class="setup-section">
              <div class="section-header">
                ...
                <span class="selection-count">{{ selectedAgents.size }} / {{ profiles.length }} 선택됨</span>
              </div>
...
const renderMarkdown = (content) => {
  ...
  // 블록 레벨 요소 앞뒤의 <br> 태그 제거
  html = html.replace(/<br>\s*(<ul|<ol|<blockquote)/g, '$1')
  html = html.replace(/(<\/ul>|<\/ol>|<\/blockquote>)\s*<br>/g, '$1')
  // <p><br> 바로 뒤에 블록 요소가 오는 경우 제거 (여분의 빈 줄로 인한)
  html = html.replace(/<p class="md-p">(<br>\s*)+(<ul|<ol|<blockquote|<pre|<hr)/g, '$2')
  // 연속 <br> 태그 제거
  html = html.replace(/(<br>\s*){2,}/g, '<br>')
  // 블록 요소 뒤에 바로 오는 단락 시작 태그 앞의 <br> 제거
...
const submitSurvey = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      addLog(`${surveyResults.value.length}건 응답 수신`)
    } else {
      throw new Error(res.error || '요청 실패')
    }
  } catch (err) {
...
const loadProfiles = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      addLog(`${profiles.value.length}명의 시뮬레이션 개체 로드 완료`)
    }
  } catch (err) {
...
/* 순서 리스트 번호 수정 - CSS 카운터를 사용하여 여러 ol의 연속 번호 지정 */
.message-text {
