        ...
        <div v-if="selectedProject" class="modal-overlay" @click.self="closeModal">
          <div class="modal-content">
            ...
            <div class="modal-playback-hint">
              <span class="hint-text">Step3 "시뮬레이션 시작"과 Step5 "심층 상호작용"은 실행 중에만 시작 가능하며, 히스토리 재생을 지원하지 않습니다</span>
            </div>
          </div>
        </div>
      </Transition>
...
const containerStyle = computed(() => {
  ...
  // 실제 필요 높이 계산: 행수 * 카드 높이 + (행수-1) * 간격 + 약간의 하단 여백
  ...
  observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        ...
        // 이전 디바운스 타이머 제거 (새로운 스크롤 의도가 이전 것을 덮어씀)
        ...
        // 디바운스로 상태 전환 지연, 빠른 깜빡임 방지
        // 펼칠 때는 짧은 지연(50ms), 접을 때는 긴 지연(200ms)으로 안정성 증가
        ...
        expandDebounceTimer = setTimeout(() => {
          ...
          // 대기 중인 상태가 여전히 실행 필요한지 확인 (후속 스크롤에 의해 덮어써졌을 수 있음)
...
// 라우트 변화 감시, 홈페이지로 돌아올 때 데이터 다시 로드
...
.grid-pattern {
  ...
  /* 좌상단부터 배치, 높이 변화 시 하단에서만 확장하여 기존 그리드 위치에 영향 없음 */
