          ...
          >
            ...
            <div class="timeline-card">
              ...
              <div class="card-body">
                ...
                <!-- 일반 폴백: 알 수 없는 타입이거나 content가 있지만 위에서 처리되지 않은 경우 -->
...
const phase = ref(0) // 0: 시작 전, 1: 실행 중, 2: 완료됨
...
// 시뮬레이션 경과 시간 포맷 (라운드와 라운드당 분 수로 계산)
...
const doStartSimulation = async () => {
  ...
  // 먼저 모든 상태를 초기화하여 이전 시뮬레이션의 영향을 받지 않도록 함
  resetAllState()
  ...
  try {
    ...
    if (res.success && res.data) {
      if (res.data.force_restarted) {
        addLog('✓ 이전 시뮬레이션 로그를 정리하고 시뮬레이션을 재시작합니다')
      }
...
// 각 플랫폼의 이전 라운드를 추적하여 변경 감지 및 로그 출력
...
const fetchRunStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // 각 플랫폼의 라운드 변경을 개별적으로 감지하고 로그 출력
...
const checkPlatformsCompleted = (data) => {
  ...
  // 하나 이상의 플랫폼이 완료되면 모든 활성화된 플랫폼이 완료되었는지 확인
...
const handleNextStep = async () => {
  ...
  if (isGeneratingReport.value) {
    addLog('보고서 생성 요청이 전송되었습니다. 잠시만 기다려주세요...')
    return
  }
