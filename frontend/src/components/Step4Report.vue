...
const toggleRawResult = (timestamp, event) => {
  ...
  // DOM 업데이트 후 스크롤 위치 조정하여 버튼을 같은 위치에 유지
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      if (answerMatch) {
        ...
        if (!twitterMatch && redditMatch) {
          // Reddit 답변만 있음, 플레이스홀더 텍스트가 아닌 경우에만 기본 표시용으로 복사
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    const getAnswerForQuestion = (interview, qIdx, platform) => {
      ...
      // 분할 실패: 첫 번째 질문은 전체 응답 반환, 나머지는 빈 값 반환
      return qIdx === 0 ? answer : ''
    }
    
    // 특정 질문에 이중 플랫폼 응답이 있는지 확인 (플레이스홀더 텍스트 필터링)
    const hasMultiplePlatforms = (interview, qIdx) => {
      ...
      // 두 플랫폼 모두 실제 응답(플레이스홀더가 아닌)이 있고 내용이 다름
              ...
              answerText && h('div', { class: ['qa-answer', { 'answer-placeholder': isPlaceholder }] }, [
                ...
                h('div', { class: 'qa-content' }, [
                  h('div', { class: 'qa-answer-header' }, [
                    ...
                    // 이중 플랫폼 전환 버튼 (실제 이중 플랫폼 응답이 있을 때만 표시)
...
const activeStep = computed(() => {
  ...
  // 그 외에는 첫 번째 단계 반환
  return steps[0] || { noLabel: '--', title: '대기 중', status: 'todo', meta: '' }
})
...
const renderMarkdown = (content) => {
  ...
  // 앞의 2단계 제목(## xxx) 제거, 섹션 제목이 이미 외부에 표시됨
      ...
      if (newLogs.length > 0) {
        newLogs.forEach(log => {
          ...
          if (log.action === 'report_complete') {
            ...
            currentSectionIndex.value = null  // loading 상태 확실히 제거
        ...
        nextTick(() => {
          if (rightPanel.value) {
            // 작업 완료 시 상단으로 스크롤; 그 외에는 하단으로 스크롤하여 최신 로그 추적
            if (isComplete.value) {
...
const extractFinalContent = (response) => {
  ...
  // "最终答案:" 뒤의 내용 찾기 시도 (서버 응답의 중국어 마커)
