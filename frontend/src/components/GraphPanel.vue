    ...
    <div class="graph-container" ref="graphContainer">
      ...
      <div v-if="graphData" class="graph-view">
        ...
        <div v-if="showSimulationFinishedHint" class="graph-building-hint finished-hint">
          ...
          <span class="hint-text">일부 콘텐츠가 아직 처리 중입니다. 잠시 후 수동으로 그래프를 새로고침하세요</span>
...
const expandedSelfLoops = ref(new Set()) // 펼쳐진 셀프 루프 항목
...
const renderGraph = () => {
  ...
  const edgePairCount = {}
  const selfLoopEdges = {} // 노드별로 그룹화된 셀프 루프 엣지
  const tempEdges = edgesData
    .filter(e => nodeIds.has(e.source_node_uuid) && nodeIds.has(e.target_node_uuid))
  
  // 각 노드 쌍 간의 엣지 수 집계, 셀프 루프 엣지 수집
  tempEdges.forEach(e => {
  ...
  // 각 노드 쌍의 현재 처리 중인 엣지 순번 기록
  const edgePairIndex = {}
  const processedSelfLoopNodes = new Set() // 이미 처리된 셀프 루프 노드
  ...
  tempEdges.forEach(e => {
    ...
    if (isSelfLoop) {
      // 셀프 루프 엣지 - 각 노드당 하나의 병합된 셀프 루프만 추가
      ...
      edges.push({
        ...
        rawData: {
          ...
          selfLoopEdges: allSelfLoops // 모든 셀프 루프 엣지의 상세 정보 저장
        }
      })
      return
    }
    ...
    // 엣지 방향이 정규화된 방향과 일치하는지 판단 (소스UUID < 대상UUID)
    const isReversed = e.source_node_uuid > e.target_node_uuid
    
    // 곡률 계산: 다중 엣지일 때 분산, 단일 엣지는 직선
    let curvature = 0
    if (totalCount > 1) {
      // 곡률 균등 분배, 명확한 구분 보장
      ...
      // 엣지 방향이 정규화 방향과 반대인 경우 곡률 반전
      // 이를 통해 모든 엣지가 동일한 좌표계에서 분포되어 방향 차이로 인한 겹침 방지
      if (isReversed) {
        curvature = -curvature
      }
    }
  ...
  const simulation = d3.forceSimulation(nodes)
    .force('link', d3.forceLink(edges).id(d => d.id).distance(d => {
      ...
      // 기본 거리 150, 엣지가 하나 늘어날 때마다 40 증가
    ...
    // 중심 방향 인력 추가, 독립 노드 그룹이 중심 영역에 모이도록
  ...
  const getLinkPath = (d) => {
    ...
    // 셀프 루프 감지
    if (d.isSelfLoop) {
      // 셀프 루프: 노드에서 출발하여 다시 돌아오는 원호 그리기
      const loopRadius = 30
      ...
      const y2 = sy + 4
      // 원호를 사용하여 셀프 루프 그리기 (sweep-flag=1 시계 방향)
    ...
    // 연결선 방향에 수직인 오프셋, 거리 비율에 따라 계산하여 곡선이 명확히 보이도록
    // 엣지가 많을수록 거리 대비 오프셋 비율 증가
  ...
  const node = nodeGroup.selectAll('circle')
    ...
    .call(d3.drag()
      ...
      .on('drag', (event, d) => {
        // 실제 드래그가 시작되었는지 감지 (이동 거리가 임계값 초과)
  ...
  simulation.on('tick', () => {
    ...
    // 엣지 레이블 위치 업데이트 (회전 없이 수평 표시로 더 선명하게)
