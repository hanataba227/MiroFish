      ...
      <div class="header-center">
        <div class="view-switcher">
          <button 
            ...
            @click="viewMode = mode"
          >
            {{ { graph: '그래프', split: '이중 패널', workbench: '워크벤치' }[mode] }}
          </button>
        </div>
      </div>
 ...
 * 사용자가 Step 3에서 Step 2로 돌아올 때 시뮬레이션 종료를 기본으로 함
 */
const checkAndStopRunningSimulation = async () => {
  if (!currentSimulationId.value) return
  
  try {
    // 먼저 시뮬레이션 환경이 활성 상태인지 확인
    const envStatusRes = await getEnvStatus({ simulation_id: currentSimulationId.value })
    
    if (envStatusRes.success && envStatusRes.data?.env_alive) {
      addLog('시뮬레이션 환경이 실행 중인 것을 감지했습니다. 종료 중...')
      
      // 시뮬레이션 환경을 정상적으로 종료 시도
      try {
      ...
      if (simRes.success && simRes.data?.status === 'running') {
        addLog('시뮬레이션이 실행 중 상태로 감지됨, 중지 중...')
        await forceStopSimulation()
      }
    }
  } catch (err) {
...
onMounted(async () => {
  ...
  // 실행 중인 시뮬레이션 확인 및 종료 (사용자가 Step 3에서 돌아온 경우)
